...
const AdminPanel = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get("http://127.0.0.1/react/users.php");
  ...
  const deleteUser = async (email) => {
    try {
      await axios.post("http://127.0.0.1/react/delete.php", { email });
      fetchUsers();
    } catch (error) {
  ...
  const updateUser = async () => {
    try {
      await axios.post("http://127.0.0.1/react/update.php", {
